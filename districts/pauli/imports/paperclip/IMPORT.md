...
## Destination
- Hall of Canon registry layer: one registry covering agents, skills, tools with provenance fields (source, SHA, license, review state) - extends the existing skill-library registry pattern already in starnet.
...
PORT LANDED (chunk 4, 2026-09-13): sidecar/pauli-registry.js + test/pauli-registry.test.js (37 assertions) - one registry for agents/skills/tools with REQUIRED provenance (source + sourceSha + license; no-source-SHA entries rejected), draft -> reviewed -> canon ladder with reviewer trail, license law enforcement (NOASSERTION/none/AGPL on permanent hold out of canon), version-superseding re-registration with full provenance history, JSONL-replay persistence. Bundled skill library registration LANDED (chunk 6, 2026-09-13): sidecar/pauli-canon-sync.js + test/pauli-canon-sync.test.js (25 assertions) - syncs the bundled library (sidecar/skills/library, 91 skills - the "370" earlier in this spec was wrong, that was the model-slug count) into the registry with repo-SHA provenance, per-skill frontmatter license taking precedence (license law travels), idempotent same-SHA re-sync, version-superseding new-SHA sync.
