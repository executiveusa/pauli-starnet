...
const roster = [
  ...
  { id: 'impact-strategist', name: 'Impact Strategist', specialtyId: 'strategist' },
  { id: 'impact-envoy', name: 'Impact Envoy', specialtyId: 'envoy' },
  { id: 'impact-legal', name: 'Impact Paralegal', specialtyId: 'paralegal' },
  { id: 'impact-casewriter', name: 'Case Writer', specialtyId: 'pitchwriter' },
  { id: 'impact-registrar', name: 'Relationship Registrar', specialtyId: 'registrar' },
  { id: 'impact-negotiator', name: 'Impact Negotiator', specialtyId: 'negotiator' },
  { id: 'impact-closer', name: 'Impact Closer', specialtyId: 'closer' },
  { id: 'impact-ghostwriter', name: 'Impact Ghostwriter', specialtyId: 'ghostwriter' },
...
A.eq(plan.summary.districts, 11, 'default app city has eleven districts including Pauli and Financial');
A.eq(plan.summary.buildings, 22, 'default app city has twenty-two buildings including three Pauli and six Financial buildings');
A.ok(plan.summary.roomsAdded >= 26, 'buildings plus connective corridors compile as one city');
...
const impactHQ = plan.buildings.find(b => b.templateId === 'impact_hq');
A.ok(impactHQ && impactHQ.districtId === 'impact', 'Impact HQ is physically compiled into the Impact District');
A.ok(impactHQ.agents.some(a => a.specialtyId === 'strategist'), 'Impact HQ gets a strategist from the real roster');
A.ok(impactHQ.agents.some(a => a.specialtyId === 'envoy'), 'Impact HQ gets an envoy for relationship cultivation');
A.ok(impactHQ.agents.some(a => a.specialtyId === 'paralegal'), 'Impact HQ gets a paralegal for eligibility/compliance review');
A.ok(impactHQ.agents.some(a => a.specialtyId === 'pitchwriter'), 'Impact HQ gets a case-for-funding writer');
const stewardship = plan.buildings.find(b => b.templateId === 'stewardship_house');
A.ok(stewardship && stewardship.districtId === 'impact', 'Stewardship House is physically compiled into the Impact District');
A.ok(stewardship.agents.some(a => a.specialtyId === 'registrar'), 'Stewardship House has a relationship registrar');
A.ok(stewardship.agents.some(a => a.specialtyId === 'negotiator'), 'Stewardship House has a negotiator');
A.ok(stewardship.agents.some(a => a.specialtyId === 'closer'), 'Stewardship House has a closer');
A.ok(stewardship.agents.some(a => a.specialtyId === 'ghostwriter'), 'Stewardship House has a relationship-aware writer');
